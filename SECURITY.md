...
`litvar-link` is a thin FastMCP backend in the GeneFoundry fleet. It is unauthenticated
...
```bash
gh api -X PATCH repos/berntpopp/litvar-link \
...
```bash
gh api repos/berntpopp/litvar-link --jq '.security_and_analysis'
```
