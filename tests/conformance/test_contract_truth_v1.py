"""Contract Truth v1 gate against the live LitVar MCP registry."""
...
async def test_documentation_matches_live_mcp_contract(
    ...
    def fake_service_factory() -> Never:
        service_calls.append("service_factory")
        raise AssertionError("contract discovery must not invoke the LitVar service")

    tools = await create_litvar_mcp(service_factory=fake_service_factory).list_tools()
