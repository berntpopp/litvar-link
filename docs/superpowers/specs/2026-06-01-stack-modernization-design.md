...
- **Two near-god files:** `api/client.py` (558 lines), `services/variant_service.py` (552). Four files exceed the "plan a split at ~500" guideline (those two plus `api/routes/variants.py` 395, `cli.py` 349).
...
- No new features or endpoints beyond what exists.
...
| D8 | Function-size enforcement | **ruff `C901` (≤10) + `PLR0915` (≤50 stmts)** | Bounds function size in CI without `PLR0913/0912` friction against FastAPI DI. This is a deliberate enhancement *beyond* the references (which cap files only). |
...
- `requires-python = ">=3.12"`; classifiers for 3.12/3.13/3.14 + Bio-Informatics; add `.python-version = 3.12`.
...
- **Function cap (new): ~50 lines / ≤50 statements / complexity ≤10**, enforced via ruff `C901` + `PLR0915` (turned on at the *end* of the P3 refactor, red→green). Documented in `AGENTS.md`.
...
- New `litvar_link/mcp/` package:
  ...
  - `errors.py` — maps service/client exceptions to MCP-safe error payloads (`mask_error_details=True`).
...
- **`ci.yml`** — PR + push `main`; one `quality` job: checkout → python → uv → sync → **`make ci-local`** → **`make test-cov`**.
...
### 4.9 Docker
- Update `docker/Dockerfile` to **uv-based multi-stage** on `python:3.13-slim`: builder runs `uv sync --frozen --no-dev` into a venv; slim production stage; non-root `app` user; healthcheck on `/api/health/`; gunicorn + uvicorn workers.
...
- **Done when:** docs accurately describe the P0 tooling and the (to-be) module layout; `@AGENTS.md` import resolves.
...
5. Explicit `litvar_link/mcp/` facade + `tools/`; remove `FastMCP.from_fastapi` path + `mcp_custom_names`; fix tool-name bug; add descriptions/instructions.
6. CLI argparse → typer (`cli:app`), split into sub-command modules.
7. Reorganize `tests/` → `tests/unit/` + `tests/integration/`; **switch on `C901` + `PLR0915`**; confirm coverage ≥ 90.
- **Done when:** every production file < 600 (most < 400), no function trips `C901`/`PLR0915`, all four DRY clusters removed, both latent bugs fixed, all 5 MCP tools resolve with correct names, `make ci-local` + `make test-cov` green.
...
- No production Python file > 600 lines (most < 400); no function trips `C901`/`PLR0915`.
...
- Tooling, packaging, CI, docs, and governance match the sibling house style; deliberate divergences (function-size rules, coverage floor 90) are documented in `AGENTS.md`.
